...
namespace itis {
  ...
  /*
   * Краткое описание работы метода: идём по дереву и сверяем символы суффикса с имеющимися. Когда находим несовпадение,
   * есть два случая:
    ...
    while (suffix_compare_index < static_cast<int>(str_.length()) && !isFinished) {
      ...
      for (; edge_compare_index < edge[1]; edge_compare_index++, suffix_compare_index++) {
        ...
        if (str_[suffix_compare_index] != str_[edge_compare_index]) {
          ...
          // старый узел мог ссылаться на какой-то другой, значит новый тоже должен, чтобы сохранить стуктуру
  ...
  // Описание: добавляем к строке символ страж $, создаем корневой узел, добавляем все суффиксы в порядке уменьшения
  // их длины, начиная с самого большого
   ...
   * реализации используется очередь. Алгоритм запускается с корня
   */
  ...
  /*
   * Алгоритм очень прост - идем по суффиксному дереву и сравниваем символы подстроки. Нашли несовпадение - подстроки
   * нет, подстрока закончилась - все символы совпали -> подстрока есть
   ...
   * дерева следует, что каждой позиции ребра соответствует префикс суффикса. Поскольку в нашем дереве есть все суффисы,
   ...
   * Поскольку у листьев ребер, ведущих к листьям, в пометке присутствует символ страж $, длину таких ребер мы будем
   * брать на 1 меньше
   */
